...
import { AlertController, InputCustomEvent, IonModal, Platform, RefresherCustomEvent, ToastController } from '@ionic/angular';
...
export class PickVerifyPage implements OnInit, OnDestroy {
  @ViewChild('square')
    ...
    await BarcodeScanner.addListener(
      ...
      async (event) => {
        this.ngZone.run(() => {
          ...
          if(firstBarcode.displayValue && !this.pickVerifyProcess.includes(firstBarcode.displayValue)){
            this.pickVerifyProcess.push(firstBarcode.displayValue);
            document.getElementById('open-modal')?.click();

            if(this.pickVerifyProcess.length > 2){
